...
import { getProposal, updateProposal } from "../../api/proposalApi";
...
function ProposalEdit() {
  ...
  const loadProposal = async () => {
    try {
      ...
      const canEdit =
        proposal &&
        Number(user.id) === Number(proposal.userId) &&
        proposal.status === "DISCUSSION";

      if (!canEdit) {
        setError("현재 상태에서는 제안을 수정할 수 없습니다.");
        return;
      }
  ...
  const validate = () => {
    ...
    if (!form.proposedWord.trim()) {
      found.proposedWord = "제안할 신조어를 입력해 주세요.";
    }

    if (!form.meaning.trim()) {
      found.meaning = "신조어의 의미를 입력해 주세요.";
    }

    if (!form.example.trim()) {
      found.example = "사용 예시를 입력해 주세요.";
    }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (err.status === 403) {
        ...
        setErrors({
          form: err.message || "신조어 제안을 수정하지 못했습니다.",
        });
      }
    } finally {
      setSubmitting(false);
      ...
      <main className="proposal-edit">
        <div className="proposal-edit-inner">
          <div className="proposal-edit-error">
            ...
            <p>{error || "신조어 제안을 불러오지 못했습니다."}</p>

            <Link to={`/proposals/${id}`}>제안 상세로 돌아가기</Link>
          </div>
        </div>
      </main>
    );
  ...
  return (
    <main className="proposal-edit">
      <div className="proposal-edit-inner">
        <Link to={`/proposals/${id}`} className="proposal-edit-back">
          ← 제안 상세로 돌아가기
        </Link>
        ...
        <section className="proposal-edit-card">
          <div className="proposal-edit-header">
            ...
            <p>등록한 신조어 제안의 내용을 수정할 수 있습니다.</p>
          </div>

          {errors.form && (
            <div className="proposal-edit-form-error">{errors.form}</div>
          )}
          ...
          >
            <div className="proposal-edit-field">
              <label htmlFor="proposedWord">제안할 신조어 *</label>
            ...
            <div className="proposal-edit-field">
              ...
              {errors.meaning && (
                <p className="proposal-edit-error-text">{errors.meaning}</p>
              )}
            </div>

            <div className="proposal-edit-field">
              <label htmlFor="example">사용 예시 *</label>
              ...
              {errors.example && (
                <p className="proposal-edit-error-text">{errors.example}</p>
              )}
            </div>
            ...
            <div className="proposal-edit-field">
              <label htmlFor="sourceDescription">출처 / 유래</label>
            ...
            <div className="proposal-edit-actions">
              <Link to={`/proposals/${id}`} className="proposal-edit-cancel">
                취소
              </Link>
